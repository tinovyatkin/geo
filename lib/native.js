...
function getDistance(start, end, accuracy = 1, precision = 0) {
...
function getBounds(points) {
  ...
  return points.reduce(
    (stats, point) => {
      ...
      return {
        maxLat: Math.max(lat, stats.maxLat),
        minLat: Math.min(lat, stats.minLat),
        maxLng: Math.max(lng, stats.maxLng),
        minLng: Math.min(lng, stats.minLng),
      };
    },
    {
      maxLat: -Infinity,
 ...
 */
function getCenterOfBounds(points) {
  const bounds = getBounds(points);
  const lat = bounds.minLat + (bounds.maxLat - bounds.minLat) / 2;
  const lng = bounds.minLng + (bounds.maxLng - bounds.minLng) / 2;
  return {
